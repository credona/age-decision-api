...
CHANGELOG_SECTION_ITEMS: tuple[str, ...] = (
    "Published Docker images from version tags only; pull request workflows no longer publish Docker images.",
    "Release workflow builds GitHub release description from the matching <code>CHANGELOG.md</code> section.",
    "Release workflow validates the Git tag matches <code>project.json</code> and that exactly one GHCR package version carries that tag.",
